...
function MiniCard({ called }: { called?: boolean[] }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 1.5, width: 120 }}>
      {BINGO_HEADERS.map((h) => (
        <div key={h} style={{ background: "#1e40af", color: "#93c5fd", fontSize: 9, fontWeight: 700, textAlign: "center", padding: "2px 0", borderRadius: 2 }}>{h}</div>
      ))}
      {CARD_NUMS.flat().map((n, i) => {
        ...
        return (
          <div key={i} style={{
            background: (isHit || isFree) ? "#1d4ed8" : "rgba(255,255,255,0.06)",
            color: (isHit || isFree) ? "#fff" : "#94a3b8",
            fontSize: 8, textAlign: "center", padding: "3px 0",
            borderRadius: 2, fontWeight: isHit ? 700 : 400,
            position: "relative"
          }}>
            {isFree ? <span style={{ color: "#fbbf24" }}>★</span> : n}
            {isHit && !isFree && <span style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, borderRadius: 2, background: "rgba(59,130,246,0.35)" }} />}
          </div>
        );
      })}
    </div>
...
export function LayoutA() {
  return (
    <div style={{ width: 390, height: 760, background: "#0f1117", display: "flex", flexDirection: "column", fontFamily: "system-ui", color: "#fff", overflow: "hidden" }}>
      ...
      {/* ── LAYOUT A: Both cards side-by-side strip at TOP ── */}
      <div style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
          <span style={{ fontSize: 9, color: "#60a5fa", fontWeight: 700, letterSpacing: 1 }}>ካርድ 1</span>
          <MiniCard called={hitA} />
        </div>
        <div style={{ width: 1, background: "rgba(255,255,255,0.1)", alignSelf: "stretch" }} />
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
          <span style={{ fontSize: 9, color: "#a78bfa", fontWeight: 700, letterSpacing: 1 }}>ካርድ 2</span>
          <MiniCard called={hitB} />
        </div>
      </div>
      ...
      <div style={{ position: "absolute", bottom: 55, left: 0, right: 0, textAlign: "center", fontSize: 10, color: "#22c55e", fontWeight: 700, pointerEvents: "none" }}>
        ✦ አማራጭ A — ካርዶቹ ከላይ ጎን ለጎን ✦
      </div>
    </div>
  );
}
